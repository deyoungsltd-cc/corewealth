    ...
    <div className="space-y-5">
      ...
      <div className="bg-card border border-border rounded-xl p-5 space-y-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
          {CRYPTOS.map((c, i) => (
            <button key={c.id} onClick={() => setSelected(i)} className={`flex items-center gap-3 p-3 rounded-xl border transition-all ${selected === i ? 'border-[#7C3AED] bg-[#7C3AED]/10' : 'border-border bg-white/[0.02] hover:border-white/20'}`}>
      ...
      <div className="bg-card border border-border rounded-xl p-5 space-y-4">
        ...
        <div><label className="block text-gray-300 text-sm font-medium mb-1.5">Amount (USD)</label>
          ...
          <div className="flex gap-2 mt-2">{QUICK_AMOUNTS.map(a => (<button key={a} onClick={() => setAmount(String(a))} className={`flex-1 py-2 rounded-lg text-xs font-semibold transition-all ${amount === String(a) ? 'bg-[#7C3AED] text-white' : 'bg-white/5 text-gray-400 hover:text-white'}`}>${a.toLocaleString()}</button>))}</div>
        </div>
        <div><label className="block text-gray-300 text-sm font-medium mb-1.5">Payment Method</label>
          <div className="space-y-2">
            {[{ id: 'debit', label: 'CoreWealth Debit Card', sub: '**** 4521' }, { id: 'credit', label: 'CoreWealth Credit Card', sub: '**** 7890' }].map(m => (
              <button key={m.id} onClick={() => setPayMethod(m.id)} className={`w-full flex items-center gap-3 p-3 rounded-xl border transition-all text-left ${payMethod === m.id ? 'border-[#7C3AED] bg-[#7C3AED]/10' : 'border-border hover:border-white/20'}`}>
                <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-[#7C3AED] to-[#6D28D9] flex items-center justify-center"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="1" y="4" width="22" height="16" rx="2" ry="2" /><line x1="1" y1="10" x2="23" y2="10" /></svg></div>
      ...
      <div className="bg-card border border-border rounded-xl p-5 space-y-3">
        ...
        <div className="space-y-2">
          ...
          <div className="flex justify-between text-sm"><span className="text-gray-500">You receive</span><span className="text-[#A78BFA] font-medium">{coinAmount.toFixed(6)} {crypto.symbol}</span></div>
          <div className="border-t border-border pt-2"><div className="flex justify-between text-sm"><span className="text-gray-500">Network fee</span><span className="text-white">${networkFee.toFixed(2)}</span></div><div className="flex justify-between text-sm mt-1"><span className="text-gray-500">Processing fee (1.5%)</span><span className="text-white">${processingFee.toFixed(2)}</span></div><div className="flex justify-between text-sm font-bold mt-2 pt-2 border-t border-border"><span className="text-white">Total</span><span className="text-[#7C3AED]">${total.toFixed(2)}</span></div></div>
        </div>
      </div>

      <button onClick={handleBuy} disabled={loading || usdAmount < 10} className="w-full bg-[#7C3AED] hover:bg-[#6D28D9] disabled:opacity-50 text-white font-semibold py-3.5 rounded-xl transition-colors text-sm shadow-[0_4px_20px_rgba(124,58,237,0.3)]">{loading ? 'Processing...' : `Buy ${crypto.symbol}`}</button>
      <ChatWidget />
    </div>
  );
}