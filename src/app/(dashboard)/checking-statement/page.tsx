...
interface Transaction { id: string; type: string; amount: number; status: string; description?: string; createdAt: string; balance?: number; }

export default function CheckingStatementPage() {
  ...
  let bal = 0;
  for (const t of [...filtered].reverse()) { bal += (t.type === 'credit' || t.type === 'deposit' ? t.amount : -t.amount); runningBalances.unshift(bal); }
  ...
  return (
    <div className="space-y-5">
      <div className="flex gap-3 flex-wrap">
        <div className="bg-card border border-border rounded-xl p-4 flex-1 min-w-[140px]"><p className="text-gray-500 text-xs">Opening Balance</p><p className="text-white font-bold text-lg mt-1">${runningBalances[0]?.toFixed(2) || '0.00'}</p></div>
        <div className="bg-card border border-border rounded-xl p-4 flex-1 min-w-[140px]"><p className="text-gray-500 text-xs">Total Credits</p><p className="text-green-400 font-bold text-lg mt-1">+${totalCredits.toFixed(2)}</p></div>
        <div className="bg-card border border-border rounded-xl p-4 flex-1 min-w-[140px]"><p className="text-gray-500 text-xs">Total Debits</p><p className="text-red-400 font-bold text-lg mt-1">-${totalDebits.toFixed(2)}</p></div>
      </div>
