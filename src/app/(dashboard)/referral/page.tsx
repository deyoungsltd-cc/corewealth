...
export default function ReferralPage() {
  ...
  const baseUrl = typeof window !== 'undefined'
    ...
    : (process.env.NEXT_PUBLIC_APP_URL || 'https://corewealthbank.com');
  ...
  const shareText = `Join me on CoreWealth Bank and start earning daily returns! Use my referral link: ${referralLink}`;

  const totalEarnings = data?.totalCommissionsEarned ?? 0;
  const totalReferrals = data?.totalReferrals ?? 0;
  const funnel = data?.funnel ?? { totalReferred: 0, kycCompleted: 0, firstDeposit: 0 };
    ...
    <div className="space-y-5">
      ...
      <div className="bg-card border border-border rounded-xl p-5">
        ...
        <div className="flex gap-3">
          <a href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(shareText)}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 bg-[#1a1a1a] hover:bg-white/10 border border-border text-gray-300 px-4 py-2.5 rounded-lg transition-colors">
            <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" /></svg>
            <span className="text-xs font-medium">X</span>
          </a>
          <a href={`https://wa.me/?text=${encodeURIComponent(shareText)}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 bg-[#1a1a1a] hover:bg-green-500/20 border border-border text-gray-300 px-4 py-2.5 rounded-lg transition-colors">
          ...
          <a href={`https://t.me/share/url?url=${encodeURIComponent(referralLink)}&text=${encodeURIComponent('Join me on CoreWealth Bank!')}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 bg-[#1a1a1a] hover:bg-blue-500/20 border border-border text-gray-300 px-4 py-2.5 rounded-lg transition-colors">
