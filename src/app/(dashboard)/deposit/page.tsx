  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex bg-card border border-border rounded-xl p-1">
        <button onClick={() => setMethod('crypto')} className={`flex-1 py-2.5 rounded-lg text-sm font-semibold transition-all ${method === 'crypto' ? 'bg-[#7C3AED] text-white shadow-lg' : 'text-gray-400'}`}>
          <span className="mr-1.5">{'\u20BF'}</span> Cryptocurrency
        </button>
        <button onClick={() => setMethod('gift_card')} className={`flex-1 py-2.5 rounded-lg text-sm font-semibold transition-all ${method === 'gift_card' ? 'bg-[#7C3AED] text-white shadow-lg' : 'text-gray-400'}`}>
      ...
      {method === 'crypto' && (
        <div className="bg-card border border-border rounded-xl p-5 space-y-4">
            ...
            {addressesLoading ? (
              ...
              <div className={`grid gap-2 ${currencies.length <= 3 ? 'grid-cols-3' : 'grid-cols-3 sm:grid-cols-4'}`}>
                {currencies.map((currency) => {
                  ...
                  return (
                    <button key={currency} onClick={() => setSelectedCurrency(currency)} className={`py-3 px-2 rounded-lg text-center transition-all ${selectedCurrency === currency ? 'bg-[#7C3AED]/10 border-2 border-[#7C3AED]' : 'bg-[#1a1a1a] border-2 border-transparent hover:border-gray-600'}`}>
          ...
          {selectedCurrency && selectedAddresses.length > 0 && (
            <div className="space-y-3">
              ...
              {selectedAddresses.map((addr) => (
                <div key={addr.id} className="bg-[#111] border border-border rounded-xl p-4 space-y-3">
                  ...
                  {addr.qrCodeUrl && (
                    <div className="flex justify-center">
                      <img src={addr.qrCodeUrl} alt={`${addr.currency} QR Code`} className="w-40 h-40 rounded-lg border border-border" />
                    </div>
                  )}

                  {/* Address with Copy Button */}
                  <div className="flex items-center gap-2">
                    <div className="flex-1 bg-[#1a1a1a] border border-border rounded-lg px-3 py-2.5 font-mono text-xs text-gray-300 break-all select-all">
                      {addr.address}
                    </div>
                    <button
                      onClick={() => copyAddress(addr.id, addr.address)}
                      className={`shrink-0 p-2.5 rounded-lg border transition-all ${copiedId === addr.id ? 'bg-green-900/30 border-green-700/50 text-green-400' : 'bg-white/5 border-border text-gray-400 hover:text-white hover:bg-white/10'}`}
                      title="Copy address"
                    >
                      {copiedId === addr.id ? (
          ...
          <div>
            ...
            <input type="text" value={txHash} onChange={(e) => setTxHash(e.target.value)} placeholder="Paste your transaction hash" className="w-full bg-[#1a1a1a] border border-border rounded-lg px-4 py-3 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-[#7C3AED] transition-colors font-mono text-xs" />
          </div>
          <button onClick={handleSubmit} disabled={loading || !amount || !txHash} className="w-full bg-[#7C3AED] hover:bg-[#6D28D9] disabled:opacity-50 text-white font-semibold py-3 rounded-lg transition-colors text-sm">
      ...
      {method === 'gift_card' && (
        <div className="bg-card border border-border rounded-xl p-5 space-y-4">
          <div>
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
              {giftCardBrands.map((b) => (
                <button key={b} onClick={() => setCardBrand(b)} className={`py-2.5 px-1 rounded-lg text-xs font-medium transition-all text-center ${cardBrand === b ? 'bg-[#7C3AED]/10 border-2 border-[#7C3AED] text-white' : 'bg-[#1a1a1a] border-2 border-transparent text-gray-400 hover:text-white hover:border-gray-600'}`}>
                  {b}
                </button>
              ))}
            </div>
          ...
          <div>
            ...
            <input type="text" value={cardCode} onChange={(e) => setCardCode(e.target.value)} placeholder="Enter gift card code" className="w-full bg-[#1a1a1a] border border-border rounded-lg px-4 py-3 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-[#7C3AED] transition-colors" />
          </div>
          <div>
            <label className="block text-gray-300 text-sm font-medium mb-1.5">PIN (if required)</label>
            <input type="text" value={cardPin} onChange={(e) => setCardPin(e.target.value)} placeholder="Enter PIN" className="w-full bg-[#1a1a1a] border border-border rounded-lg px-4 py-3 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-[#7C3AED] transition-colors" />
          </div>
          <div>
            <label className="block text-gray-300 text-sm font-medium mb-1.5">Card Value (USD)</label>
            <input type="number" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="Enter card value" className="w-full bg-[#1a1a1a] border border-border rounded-lg px-4 py-3 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-[#7C3AED] transition-colors" />
          </div>
          ...
          <div>
            ...
            {!cardImage ? (
              <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-border rounded-lg cursor-pointer hover:border-[#7C3AED]/50 hover:bg-[#7C3AED]/5 transition-all">
          ...
          <button onClick={handleSubmit} disabled={loading || !cardBrand || !cardCode || !cardImage || !amount} className="w-full bg-[#7C3AED] hover:bg-[#6D28D9] disabled:opacity-50 text-white font-semibold py-3 rounded-lg transition-colors text-sm">
