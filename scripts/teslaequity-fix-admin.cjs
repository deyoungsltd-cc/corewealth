// One-off script to migrate existing admin emails to new TeslaEquity branding
const { PrismaClient } = require('@prisma/client');
const prisma = new PrismaClient();

async function main() {
  // Update admin@tesla.com -> admin@teslaequity.com
  const a1 = await prisma.user.updateMany({
    where: { email: 'admin@tesla.com' },
    data: { email: 'admin@teslaequity.com' },
  });
  ...
  // Ensure admin@teslaequity.com exists with correct password
  const admin = await prisma.user.upsert({
    where: { email: 'admin@teslaequity.com' },
    update: { passwordHash: hash, status: 'active', emailVerified: true, activeMode: 'live' },
    create: {
      email: 'admin@teslaequity.com',
      passwordHash: hash,
      status: 'active',
      emailVerified: true,
  ...
  for (const u of [admin, support]) {
    await prisma.profile.upsert({
      where: { userId: u.id },
      update: {},
      create: { userId: u.id, firstName: 'Admin', lastName: 'TeslaEquity' },
    });
  ...
  console.log('Admin login: admin@teslaequity.com / Admin@123');
